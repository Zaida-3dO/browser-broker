...
| **73** | **`stripCommentsKeepingLines` is not string-aware, so any URL in a string literal truncates it** (`scripts/check-arbitration.mjs`). `//` inside a literal is blanked as a line comment, leaving the string unterminated, which makes `matchingBracket` return `null` and **silently drops the enclosing region** — the guard then scans less than it reports scanning. Confirmed by execution: a literal holding any scheme-and-double-slash address comes back truncated at the slashes, and the literal is gone. **The failure direction is safe** (a dropped exemption means more firing, not less), which is why this is a row rather than an incident — but it is the silently-degrading class the file's own header warns about, and a change to either scan could flip which direction it fails in | 32 | `todo` |
