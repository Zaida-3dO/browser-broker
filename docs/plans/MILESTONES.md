...
| **2** | Publish the repository and protect `main` — linear history, no force-push, no deletions, pull request required. **Ships the `LICENSE` file** (MIT, `DECISIONS.md` §13e) | 1 | `done` |
...
| **73** | **The comment strippers were not string-aware, so a pair of slashes in a string literal truncated it** (`scripts/check-arbitration.mjs`). Fixed: both strippers are now one walk that delegates string tracking to `endOfString`, so the stripper and the bracket matchers agree by construction rather than by coincidence. **The row understated this in both directions, measured on unmodified source.** Unsafe: `stringLiterals` strips comments first, so a scheme-bearing literal swallowed the next literal on its line and a forbidden-SQL literal became invisible to rule one scan A — a hidden violation, not the over-firing the row assumed. False-positive: `endOfString` stopped at the next backtick, so a nested template literal ended an after-commit region early and exposed genuine after-commit work to rule three scan B. Both are covered by tests, all four revert-mutations killed. **Regex literals remain out of scope** and are declared as such in the code: telling a regex from a division needs a real parse, and no scanned file holds one | 32 | `done` |
...
| **72** | **Two intermittent failures, neither reproducible in isolation** — `tests/browser/sign-in-evidence.test.ts` and a capture case in `tests/adapter/conformance/controls.test.ts`. Both were sighted once during a build, passed alone under repetition, and passed on a pristine tree, so **neither has been shown to be pre-existing and neither has been shown to be caused**. Delivers a diagnosis: whether they share a cause, and whether it is shared temporary state between cases. **A test that fails once in six and then not at all is the shape that gets attributed to the diff in front of it**. **Answered: they do NOT share a cause, and no state is shared between cases** — what they share is that `node --test` runs files concurrently, so both are load-dependent and neither can be found by repeating a single case. The browser one is a teardown that could not wait (`EPERM` from a bare `rmSync`, attributed by the runner to the file rather than a test). **The conformance one was a real defect on the command line**: a base64url lease key beginning with `--` was parsed as the next option, so about one key in 6,250 was swallowed and refused as missing. `DECISIONS.md` §13g | 60 | `done` |
...
| **63** | **Dialog handling as an action on `browser_act`** — answer or dismiss a native dialog. **Measured at 8 calls, and it is here on consequence rather than frequency**: an unhandled dialog blocks its tab and burns the lease (`SCHEMA.md` §3.8) | 22 | `done` |
| **64** | **Batch fill, and drag-and-drop, as actions on `browser_act`.** Batch fill is measured at **78 calls across 35 sessions**; **drag and drop measured zero calls over a month and are unexercised** — folded in at low priority and recorded as such (`SCHEMA.md` §3.8) | 22 | `done` |
| **23** | Read: snapshot by default; console, network and cookie summary on request — all path-returning; **cookie values never returned** | 21 | `done` |
| **24** | Evaluate, with an inline byte cap and spill-to-path | 21 | `done` |
...
| **44** | **The setup handshake, run on every spawn.** `broker init` runs it explicitly and every process that opens the store runs it before doing anything else: step the schema, confirm the two browser rows, **create a profile that is absent and use one that is present — never recreate**, and report which profiles it created against which it found. Idempotent. Refuses with a named reason when the profile root is unwritable or another process holds a profile's lock (`SCHEMA.md` §1.2d) | 20 |`done` |
...
| PR | Delivers | Needs | Status |
|---|---|---|---|
| **25** | Adapter contract and the shared conformance harness — **an unregistered adapter fails the suite** | 10 | `done` |
| **27** | **Tool surface over stdio — the primary route.** The service is spawned by its caller, serves that session and exits with it | 25 | `done` |
| **29** | Command-line adapter (`broker claim` / `status` / `release` / the operations commands) — **in process, because there is nothing else for a command to talk to** — and the parity proof. Ships `broker claim --wait`, which polls at just under the lease lifetime | 25 | `done` |
| **30** | Parity suite green across every route: identical operations, identical refusals, **identical side-effects** | 27, 29 | `done` |
...
| **37** | **The capture telemetry rollups** — what pictures cost and what diffs did, read back from the `captures` and `comparisons` tables the schema already indexes for exactly this (`captures_taken_at`, `comparisons_source`, `comparisons_target`). The reader half of the original row, which stands on its own: it is what makes the capture policy's effect legible at all. Read by `broker captures` | 27 | `done` |
| **37a** | **The matched-pair study over real review work** — the comparison that says whether the capture policy paid for itself. **Deliberately not scheduled**, and the reason is a premise rather than a dependency: a matched pair needs real review traffic to pair, and nothing has been rolled out to generate any (`docs/ROLLOUT.md`). Scheduling it before there is traffic would produce a study of its own fixtures. **The row that unblocks this is a rollout, not a build** | 37, 38 | `open` |
| **38** | Rollout runbook: phased enablement from a zero tab budget to sole route, in the order that never leaves traffic unarbitrated. **Includes the one-time sign-in, by hand, into the window already open — nothing stopped, nothing relaunched** | 44 | `done` |
...
| PR | Delivers | Needs | Status |
|---|---|---|---|
| **40** | **Diff: a capture against the earlier capture the caller named**, producing a diff mask and a `comparisons` row. **Reuse a diff library; do not write a differ.** **A missing target returns the full screenshot with an explanation, never a refusal.** Ships the **geometry handling** — a width mismatch is reported in the result rather than pre-empted, and a full page's height is allowed to differ, with the change in page length reported as its own fact | 31, 7 |`done` |
| **41** | Changed-region extraction: connected components over the mask into bounding boxes, merged at a configurable distance, and filtered **on area with a thin-line allowance** — not on the shorter side, which discards a one-pixel line across a wide page | 40 |`done` |
| **42** | Region crops written as paths — **both the earlier crop and the new one, from the same rectangle, with padding** — plus the outlined overlay, `truncated` when the region cap bites, and `compared_against` echoed back. **All of it rides on `browser_capture`; there is no comparison tool** (`SCHEMA.md` §3.13) | 41, 27 |`done` |
| **43** | Threshold tuning: a configurable colour tolerance, a fixture set of known-clean and known-changed pairs, and a test proving a **real** change is not swallowed. Build the fixtures from thin lines, border widths, focus rings and underlines — the cases the size filter is most likely to eat | 41 |`done` |
| **48** | **A read surface for diffs** — `broker diffs`, listing them by capture, by target or by lease. The table's entire justification is that tuning reads it, so a version with nothing reading it has a justification and no evidence | 40, 29 | `done` |
...
**Which halves are built by which row, stated precisely, because the split is not obvious from the
list above.** Step 1 and step 3 are covered by the lazy global sweep (#16) in `arbitration.ts`: it
runs first on every arbitration call, expires every lapsed claim across the whole store, moves their
tabs to the state that is true of each, and schedules the close after the commit. **Step 2 is #21a**,
and it was kept as its own row rather than absorbed into this one — a requirement listed under a
`done` row is a requirement nobody schedules.

**#21a is `broker reconcile <browser>`**, and its shape is the constraint `service/tabs.ts` sets out:
the deciding is a pure function over what the browser said and what the store holds
(`decideReconciliation`), the writing takes a database handle and no session (`applyReconciliation`),
and the asking sits visibly between them in `cli/reconcile-command.ts`. Nothing holds a driver and a
database handle at once, which is what keeps §2.4b obvious rather than incidental.

**It is an administrative command, not an agent operation** (§4.3, §5.4). Closing pages across a
whole browser is browser-scoped and destructive, which §3.13 keeps off the agent surface and
`browser_scoped.never` (§7.3) makes a build rule — the same reasoning that puts reap and restart
there.

**A tab in `opening` suspends the closing half.** `reserveTab` writes its row before the browser is
asked, so between the commit and the open there is a page in the browser that no row yet names. That
page is indistinguishable from a leaked one, so while any row is `opening` nothing is closed at all,
and the run says so. The settling half is unaffected: §1.4's `CHECK ((state = 'opening') =
(driver_tab_id IS NULL))` means such a row holds no name to look for, so it can never be found
missing. The cost is that a genuinely leaked page waits for the next run; the cost of guessing is a
caller losing a tab it was just granted.
