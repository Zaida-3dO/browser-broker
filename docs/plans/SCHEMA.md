...
|---|---|---|
| `call.arguments_declared` | **A call carries only argument names the tool it names declares** (§3.1). Checked on the tool surface, before the call reaches an operation | undeclared argument, **naming the offending names and listing what the tool does take**, so the caller can correct it without a second round trip to `tools/list`. **This is the one rule about what the caller did not ask for rather than about what it asked for**, and it exists because the alternative is silence: the arguments record travels to the bridge as an opaque set of names, each operation reads the ones it knows, and a name no tool declares is therefore invisible to every layer that could object. Unguarded it is dropped while the call answers `accepted` — so a caller sending `resize` to an operation that has never declared one gets a result computed without it and no indication that anything was ignored. **That is the failure `check-argument-reachability` (§7.3) exists to prevent, arriving from the other direction**: there a declared name is read by nothing, here an undeclared one is accepted by everything, and both hand back a truthful-looking success for a request that was partly ignored. **Accepting and warning was rejected**: a warning rides on a response whose `outcome` still reads `accepted` and whose `isError` is still false, which are the two fields a client branches on, so the false conclusion would survive it. The cost is borne honestly — a caller sending a stray name is refused where it was answered, and the behaviour it believed it had is the behaviour it never had |
...
| `claim.session_bounded` | A claim carries the **identity of the session asking for it** (§1.3) — mandatory, at least one character. No maximum and no opinion on its content: the identifier is a key another system owns, so what is checkable here is only whether one arrived | session id missing, naming the argument and distinguishing an absent one from a blank one. **Checked at the service boundary, and first — ahead of every other rule on this operation**, because every other refusal records the asking session on its ledger row (§1.6) and cannot do so if the identity is what is missing. Unlike `claim.purpose_bounded` there is **no backstop**: `claims.session_id` is `TEXT NOT NULL` with no `CHECK`, and the empty string a surface produces from a missing argument satisfies `NOT NULL` — so before this rule the lease was **granted**, with a real key and a real tab, and `session_id = ''` written to the claims row and every ledger event. Nothing failed, which is why it went unseen. **The refusal row itself leaves `session_id` null** rather than writing the empty string it exists to refuse |
| `claim.browser_known` | The browser named is one of the two | unknown browser |
| `claim.browser_kind_agrees` | **A browser's stored row agrees with the kind this process configured it as** (§13i). Checked right after the claim-time `INSERT OR IGNORE`, inside the same transaction, because that statement is a no-op — and so cannot itself detect a mismatch — when a row with the name already exists | browser kind mismatch, naming the browser, the kind on the row and the kind this process configured. Two processes on one machine may hold different configurations (§13i); this is that disagreement made a nameable refusal rather than a silently adopted row of the wrong kind |
| `claim.purpose_bounded` | A claim carries a **purpose within its length bounds** (§1.3) — mandatory, three to two hundred characters | purpose out of bounds, naming the argument and whether it was missing, too short or too long. **Checked at the service boundary, before any statement reaches the store**: the column's own `CHECK` is a backstop for writers that are not the claim path, and a caller who reached it got a driver error naming a constraint — an unhandled `SqliteError` on the command line, `unexpected_failure` on the tool surface — rather than a refusal naming the argument |
...
| `navigate.wait_bounded` | How long to wait for a page is **a whole number of milliseconds from 1 to the lease lifetime** | invalid wait, naming the accepted range and the value. **Bounded by the lease rather than by a literal**: a wait outliving the lease would hold a tab past the point the service considers it reclaimable, and a ceiling derived from the setting moves with it. **A bound on the navigation, not a pause after it** — the call returns as soon as the page is there, so two calls differing only in this argument waited only as long as the page took |
...
| `capture.tier_known` | A requested tier is one of the two rungs above the default | invalid tier, naming both accepted words. **`default` is refused as a value even though it is a real rung**: it is what passing nothing gives you, and the seam's own type excludes it so that asking for it by name cannot compile. The guard exists because the surfaces carry free text — an unrecognised word would otherwise index the rung table to nothing and be handed to the downscaler as a target edge |
...
| `act.ref_shaped` | A reference **has the shape a snapshot mints** — `e14`, or `f1e23` for an element inside an iframe (§3.8) | not a reference, quoting a truncated form of what was sent and pointing at `browser_read { what: "snapshot" }`. **A conventional refusal, answered before any tab is touched.** A selector and a stale reference fail identically at resolution and differ plainly here, so the selector case is named here and `act.ref_resolves` is left to mean staleness alone |
